...
if args.save_dir:
    ...
    save_dir = f'{args.save_dir}/exp{model_file_name}{timestamp}'
...
concept_num, graph, train_loader, valid_loader, test_loader = load_dataset(
    ...
    args.batch_size, # 128
    args.graph_type, # 'Dense'
    dkt_graph_path=dkt_graph_path, # NONE
    train_ratio=args.train_ratio, # 0.6
    val_ratio=args.val_ratio, # 0.2
    shuffle=args.shuffle, # BY DEFAULT TRUE
    model_type=args.model, # 'GKT' or 'DKT'
    use_cuda=args.cuda # BOOLEAN
)

import pandas as pd
df_graph = pd.DataFrame(graph).to_csv('graph.csv', index=False)
...
if args.model == 'GKT':
    ...
    model = GKT(
        concept_num, args.hid_dim, args.emb_dim, args.edge_types, 
        args.graph_type, graph=graph, graph_model=graph_model,
