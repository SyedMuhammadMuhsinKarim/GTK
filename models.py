...
class GKT(nn.Module):
    ...
    Working:
        1. Aggregate step, as shown in Section 3.2.1 of the paper
    """
    
    def __init__(
        self, concept_num, hidden_dim, embedding_dim, edge_type_num, graph_type, 
        graph=None, graph_model=None, dropout=0.5, bias=True, binary=False, has_cuda=False
    ):
        """
        Parameters:
            concept_num: the number of concepts - nodes in the graph
            hidden_dim: the dimension of hidden representations
            embedding_dim: the dimension of concept embedding
            edge_type_num: the number of edge type
            graph_type: the type of graph
            graph: the graph structure
            graph_model: the graph neural network model
            dropout: the dropout rateuse
            bias: whether to use bias
            binary: whether to use binary cross entropy loss
            has_cuda: whether to use cuda

        Return: None
        """
        
        super(GKT, self).__init__()
        graph_type_list = ['Dense', 'Transition', 'DKT', 'PAM', 'MHA', 'VAE']
        ...
        assert graph_type in graph_type_list, 'The type of graph is not supported.'
        self.graph_type = graph_type
        
        if graph_type in graph_type_list[:3]:
        ...
        else: # graph_type in graph_type_list[3:]
        ...
        one_hot_feat = torch.eye(self.res_len * self.concept_num) # identiy matrix (2|12 * concept_num)
        ...
        self.pos_emb = PositionalEmbedding(d_model=embedding_dim, max_len=self.res_len * concept_num) # positional embedding
        ...
        # 
        self.transform = nn.Transformer(hidden_dim, hidden_dim, dropout=dropout)
        # prediction layer
    ...
    def _aggregate(self, xt, qt, ht, batch_size):
        r"""
        Description:
            function to aggregate the hidden representations of concepts
            and questions
    ...
    def _agg_neighbors(self, tmp_ht, qt):
        ...
        if self.graph_type in ['Dense', 'Transition', 'DKT', 'PAM']:
            ...
            if self.graph_type == 'MHA':
                ...
                query = self.pos_emb.forward(query)
    ...
    def _update(self, tmp_ht, ht, qt):
        ...
        # res_gru = self.gru(m_next[qt_mask].reshape(-1, self.hidden_dim), ht[qt_mask].reshape(-1, self.hidden_dim))  # [mask_num * concept_num, hidden_num]
        res_tf = self.transform(
            m_next[qt_mask].reshape(-1, self.hidden_dim), # mask_num * concept_num
            ht[qt_mask].reshape(-1, self.hidden_dim) # hidden_num
        ) 
    ...
    def _get_edges(self, masked_qt):
        r"""
        Description:
            Get edges for edge inference in VAE from question indices of all students in a batch.
            For, each student in a batch, the question indices are used to get the edges for edge inference in VAE.
            The edges are used to infer the latent variable z in VAE.
